...
const { gibVerfuegbarkeit, futureXUrls2, extrahiereZahl, extrahiereDatum } = require('./funktionen.js');
...
(async () => {
    ...
    let listVonUrlArtikel = await futureXUrls2("https://www.future-x.de/Hardware-Netzwerk/PC-Komponenten/Grafikkarten/?b2bListingView=listing&p=");
    ...
    for(let i = 0; i < listVonUrlArtikel.length; i++){ 
        ...
        try{
            ...
            if(artikel.durchschnittlicherVerbrauch && artikel.speicherKapazitaet && artikel.grafikprozessor ){
                if(artikel.imgUrl.length < 255 || artikel.produktlink.length < 255){
                    listeArtikel.push(artikel);
                    console.log(artikel);
                    // sende gecrawlten Artikel in server
                    const produktListe = { kategorie: 'Grafikkarte', value: artikel };
                    try {
