    ...
    return this.http.get<any[]>(`${this.apiURL}/Artikel2/${kategorie}?shopId=${shopId}`).pipe(
      map((data) => {
        return data.map((grafikkarteData: any) => new Grafikkarte(
          ...
          grafikkarteData.verfuegbarkeit,
